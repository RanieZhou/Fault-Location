...
SYSTEM_PROMPT = """你是配网故障定位专家助手，服务于10kV配电网的一线运维人员和调度员。
...
# 工具调用结果的格式化提示（注入到 tool message 之前）
def format_tool_result_hint(tool_name: str) -> str:
    hints = {
        "list_topologies": "以下是系统里已上传的拓扑列表：",
        "locate_fault_from_alarms": "以下是矩阵法定位结果，请基于此给出分析：",
        "infer_fault_from_event": "以下是从历史电气量数据自动推理出的定位结果，请基于此给出分析：",
        "list_topology_events": "以下是该拓扑已上传的历史电气量事件列表：",
        "search_similar_history": "以下是历史相似案例，请结合当前情况给出参考：",
        "get_node_info": "以下是节点详细信息：",
        "generate_dispatch_suggestion": "以下是派工建议，请向用户呈现：",
        "query_topology": "以下是拓扑结构：",
        "list_history_events": "以下是历史故障事件列表：",
    }
    return hints.get(tool_name, "工具调用结果：")
